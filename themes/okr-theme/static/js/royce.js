      ...
      d.feed.entry.forEach(function(f)
      {
       ...
       box.forEach(function(f){
        ...
        if (f.department=="department")
        {
          if(f.type=="o")
          {
            words="<h4>"+f.description+"</h4><div class='progress-bar-container'>"+f.key+"<div class ='progress'><div data-percentage='50%' style='width: "+(parseFloat(f.key)*100)+"%;' class='progress-bar ";
            if(parseFloat(f.key)<=0.3)
            {
              words+="progress-bar-danger";
            }
            else if (parseFloat(f.key)<=0.6)
            {
          ...
          else
          {
            ...
            container.innerHTML+="<div class ='progress-bar-title'><div class='progress-bar-text'>"+f.key+"</div></div><div class='key'><h6>"+f.description+"</h6></div>";
